...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "PmcNetwork") type = PmcNetwork;
    if(strType == "PmcNetworkVariable") type = PmcNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == PmcNetwork) s = "PmcNetwork";
    if(type == PmcNetworkVariable) s = "PmcNetworkVariable";
