...
const char * const BITCOIN_CONF_FILENAME = "pmc.conf";
const char * const BITCOIN_PID_FILENAME = "pmcd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "pmc" is a composite category enabling all Pmc-related debug output
            if(ptrCategory->count(string("pmc"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\PmcCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\PmcCore
    // Mac: ~/Library/Application Support/PmcCore
    // Unix: ~/.pmccore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PmcCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/PmcCore";
#else
    // Unix
    return pathRet / ".pmccore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty pmc.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override pmc.conf
